				...
				<form class="form-horizontal" action="{{route('tunjangan.store')}}" method="POST">	
                    ...
                    @if (isset($error_create))
                            <div style="width: 100%;color: red;text-align: center;">
                                Untuk Tunjangan Yang Jabatan <u>{{$tunjangan1->jabatan->nama_jabatan}}</u> && Golongan <u>{{$tunjangan1->golongan->nama_golongan}}</u> Status <u>{{$tunjangan1->status}}</u> Jumlah Anak <u>{{$tunjangan1->jumlah_anak}}</u> Sudah Ada 
                            </div>
                            @endif
                    </div>
					<div class="form-group">
